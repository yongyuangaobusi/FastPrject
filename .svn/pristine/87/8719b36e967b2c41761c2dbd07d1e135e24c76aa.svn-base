...
function getMapData(channel,clement,param,gs,date,fild){
...
//	}
	...
	case "init":
		...
		if(opt.type=="TP"){
			...
			getMapData("TMP","TMP","gridParam",false,"","");
		}else if(opt.type=="TQXX"){
			
			getMapData("TQXX","TQXX","gridParam",false,"","");
		}else if(opt.type=="EDA"){
			//GIS.Warning.showWarningData("nGis", gisInfo, this.gisPopFunction);
			getMapData("EDA","EDA","gridParam",false,"","");
...
//		GridPanel_PageParam.initTablebody();
	...
	function iniStationData(type){
		var url="../../static/wh/hebei/resources/business/yjxx/js/qtyb.json";
		$.ajax({
			type:'POST',
			url:url,
			dataType:"json",
