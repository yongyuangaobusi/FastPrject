...
;(function($, win, undefined) {
	var _gridParam = null;//背景场表格中被选中格子的属性
	...
	GDZZ_TOOLS.gisDisplayType=function(ele){//各要素在地图上显示的样式
		var types = {"TP24H" : ["splash","number"],"TP1H" : ["splash","number"],"TP3H" : ["splash","number"],
...
//			        	             {name : "120223", title : "静海"}, {name : "120125", title : "蓟州"}],define: "12"},
		...
		var gridToolParam = {
				...
				tools : [
				         {name : "freeArea", title: "任意区域", iconClass : "icon-renyiquyu-7",type: "trigger", solo:true,
				        	 droplist : [{name : "click", title : "单击绘制"}, {name : "free", title : "自由绘制"}, {name : "turnWind", title : "转向风绘制"}],define: "click"},
			        	 ...
			        	 {name : "brush", title: "数据刷", iconClass : "icon-shujushua-7",type: "trigger", solo:true,
				        	 droplist : [{name : "stamp", title : "盖章"}, {name : "hollow", title : "凹槽"}, {name : "bulge", title : "凸脊"}],define: "hollow"}
		...
		var synergyToolParam = {
				...
				opened : 0,
				tools : [
				         {name : "createwea", title: "生成天气现象", iconClass : "icon-icon-collaborative-tool-1", type: "button", solo:true},
				         {name : "createdis", title: "生成灾害落区", iconClass : "icon-icon-descentarea-5", type: "button", solo:true},
				         {name : "consistency", title: "要素一致", iconClass : "icon-yaosuyizhi-7", type: "button", solo:true}
				         ],
		        reCall: toolClicked
		};
...
//		var $content = $('.consistencyPanel .content:visible[ignore!=true] li');
	...
	GDZZ_TOOLS.readLagend = function(){
		$.ajax({
			type:'POST',
			url:'/Web/HB/hebei/resources/business/gdzz/main/js/readLagendData.json',
			dataType:"json",
			async:false,
