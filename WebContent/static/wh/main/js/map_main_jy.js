...
;(function($, win, undefined) {
	...
	GDZZ_TOOLS.initPage = function(){
		...
		var basicToolParam = {
			...
			right: "0px",
			name : "basicTools",
			title : "要素选择",//工具条头上显示的字
			opened : 1,//工具条是否展开 1 展开 0缩起
			tools : [
			         {name : "changePost", title: "降雨", iconClass : "icon-icon-newest-4", type: "button", solo:true},
			         {name : "focusArea", title: "轨温", iconClass : "icon-qingchuquanbusuoding-7",type: "button", solo:true},
		             {name : "clearArea", title: "风", iconClass : "icon-xiangpica-7",type: "button", solo:false}
			         ],
		...
		var stationToolParam = {
				...
				tools : [
				         ...
				         {name : "chart", title: "降雨统计", iconClass : "icon-icon-roaming-1",type: "button", solo:true}
				         ],
		        reCall: toolClicked
		};
...
//				alert('任意区域-天气现象-'+_gisParam.weather);
	...
	function showLegend(element){
		if(element){
			...
			if(name != "WEA" && name != "DIS"){
				$('#grid-legend .gdzz-legend[name='+name+']').show();
			}
		}
	}
